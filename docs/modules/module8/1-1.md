...
2. 페이지를 아래로 스크롤하여 **Previous releases** 섹션에서 `Ubuntu Server 24.04 LTS`를 선택합니다.
    ...
    ![Ubuntu Previous Releases 화면](../../assets/images/module8/1-1.png)
...
1. VirtualBox에서 `새로 만들기`를 클릭합니다.
    ...
    ![새로 만들기 클릭](../../assets/images/module8/1-2.png)
...
2. 아래와 같이 입력합니다.
    ...
    ![기본 정보 입력 화면](../../assets/images/module8/1-3.png)
...
![포트포워딩 설정 화면](../../assets/images/module8/1-4.png)
...
!!! warning "VM을 먼저 종료하세요"
    네트워크 어댑터 설정은 VM이 실행 중일 때는 변경할 수 없습니다.
    VM 터미널에서 아래 명령어로 종료한 후 진행하세요.
    ```bash
    sudo poweroff
    ```
...
먼저 아래 명령어로 네트워크 인터페이스 목록을 확인합니다.

```bash
ip addr
```

어댑터 2에 해당하는 인터페이스명을 확인합니다. 보통 `enp0s9`이지만 `enp0s8` 등 다를 수 있습니다.

!!! tip "인터페이스 확인 방법"
    `ip addr` 출력에서 `enp0s3` 다음에 나오는 인터페이스가 어댑터 2입니다.
    해당 인터페이스명을 아래 Netplan 설정에서 사용하세요.
