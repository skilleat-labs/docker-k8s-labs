# 3-3. 볼륨 실습 — emptyDir에서 PersistentVolume까지
...
### emptyDir 한계 확인 — Pod 삭제 시 데이터 소실
...
> **결론**: emptyDir은 임시 데이터나 컨테이너 간 파일 공유에 적합하지만, Pod 삭제 후에도 데이터를 유지하려면 **PersistentVolume(PV) / PVC**가 필요합니다.
...
## 4) 방법 B — 동적 프로비저닝 (StorageClass 자동 PV 생성)
...
!!! warning "Rancher Desktop — PVC가 Pending 상태로 머무는 경우"
    ...
    **PVC만 생성한 시점에서는 PV가 만들어지지 않으며, PVC 상태가 `Pending`으로 유지되는 것이 정상입니다.**
    다음 단계(5)에서 Pod를 배포하면 그때 비로소 PV가 생성되고 PVC가 `Bound`로 바뀝니다.
...
!!! note "Rancher Desktop — Pod 배포 후 PVC 상태 변화"
    Pod를 배포하면 `WaitForFirstConsumer` 모드에 따라 PV가 자동 생성되고 PVC가 `Bound`로 바뀝니다.

    ```bash
    kubectl get pvc dynamic-pvc   # Pending → Bound
    kubectl get pv                # 자동 생성된 PV 확인
    ```
