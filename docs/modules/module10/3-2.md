...
### 2) Gateway API를 지원하는 Controller 설치
...
> 1단계에서 CRD를 이미 설치했기 때문에 `--skip-crds`를 반드시 붙여야 합니다. 생략하면 충돌 오류가 발생합니다.
...
### 8) HTTPRoute — 가중치 기반 트래픽 분산 (Canary)
